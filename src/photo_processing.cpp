...
int Java_com_xinlan_imageeditlibrary_editimage_fliter_PhotoProcessing_nativeInitBitmap(
    JNIEnv *env, jobject thiz, jint width, jint height) {
  return initBitmapMemory(&bitmap, width, height);
}

void Java_com_xinlan_imageeditlibrary_editimage_fliter_PhotoProcessing_nativeGetBitmapRow(
...
void Java_com_xinlan_imageeditlibrary_editimage_fliter_PhotoProcessing_nativeSetBitmapRow(
    JNIEnv *env, jobject thiz, jint y, jintArray pixels) {
  int cpixels[bitmap.width];
...
int Java_com_xinlan_imageeditlibrary_editimage_fliter_PhotoProcessing_nativeGetBitmapWidth(
    JNIEnv *env, jobject thiz) {
  return bitmap.width;
}

int Java_com_xinlan_imageeditlibrary_editimage_fliter_PhotoProcessing_nativeGetBitmapHeight(
    JNIEnv *env, jobject thiz) {
  return bitmap.height;
}

void Java_com_xinlan_imageeditlibrary_editimage_fliter_PhotoProcessing_nativeDeleteBitmap(
    JNIEnv *env, jobject thiz) {
  deleteBitmap(&bitmap);
}

void Java_com_xinlan_imageeditlibrary_editimage_fliter_PhotoProcessing_nativeFlipHorizontally(
    JNIEnv *env, jobject thiz) {
  flipHorizontally(&bitmap, 1, 1, 1);
}

int Java_com_xinlan_imageeditlibrary_editimage_fliter_PhotoProcessing_nativeRotate90(
    JNIEnv *env, jobject thiz) {
  ...
  // All the component dimensions should have changed, so copy the correct
  // dimensions
  bitmap.width = bitmap.redWidth;
  bitmap.height = bitmap.redHeight;
}

void Java_com_xinlan_imageeditlibrary_editimage_fliter_PhotoProcessing_nativeRotate180(
    JNIEnv *env, jobject thiz) {
  rotate180(&bitmap, 1, 1, 1);
}

void Java_com_xinlan_imageeditlibrary_editimage_fliter_PhotoProcessing_nativeApplyInstafix(
    JNIEnv *env, jobject thiz) {
  applyInstafix(&bitmap);
}

void Java_com_xinlan_imageeditlibrary_editimage_fliter_PhotoProcessing_nativeApplyAnsel(
    JNIEnv *env, jobject thiz) {
  applyAnselFilter(&bitmap);
}

void Java_com_xinlan_imageeditlibrary_editimage_fliter_PhotoProcessing_nativeApplyTestino(
    JNIEnv *env, jobject thiz) {
  applyTestino(&bitmap);
}

void Java_com_xinlan_imageeditlibrary_editimage_fliter_PhotoProcessing_nativeApplyXPro(
    JNIEnv *env, jobject thiz) {
  applyXPro(&bitmap);
}

void Java_com_xinlan_imageeditlibrary_editimage_fliter_PhotoProcessing_nativeApplyRetro(
    JNIEnv *env, jobject thiz) {
  applyRetro(&bitmap);
}

void Java_com_xinlan_imageeditlibrary_editimage_fliter_PhotoProcessing_nativeApplyBW(
    JNIEnv *env, jobject thiz) {
  applyBlackAndWhiteFilter(&bitmap);
}

void Java_com_xinlan_imageeditlibrary_editimage_fliter_PhotoProcessing_nativeApplySepia(
    JNIEnv *env, jobject thiz) {
  applySepia(&bitmap);
}

void Java_com_xinlan_imageeditlibrary_editimage_fliter_PhotoProcessing_nativeApplyCyano(
    JNIEnv *env, jobject thiz) {
  applyCyano(&bitmap);
}

void Java_com_xinlan_imageeditlibrary_editimage_fliter_PhotoProcessing_nativeApplyGeorgia(
    JNIEnv *env, jobject thiz) {
  applyGeorgia(&bitmap);
}

void Java_com_xinlan_imageeditlibrary_editimage_fliter_PhotoProcessing_nativeApplySahara(
    JNIEnv *env, jobject thiz) {
  applySahara(&bitmap);
}

void Java_com_xinlan_imageeditlibrary_editimage_fliter_PhotoProcessing_nativeApplyHDR(
    JNIEnv *env, jobject thiz) {
  applyHDR(&bitmap);
}

int Java_com_xinlan_imageeditlibrary_editimage_fliter_PhotoProcessing_nativeLoadResizedJpegBitmap(
...
int Java_com_xinlan_imageeditlibrary_editimage_fliter_PhotoProcessing_nativeResizeBitmap(
...
Java_com_xinlan_imageeditlibrary_editimage_fliter_PhotoProcessing_freeBeautifyMatrix(
    JNIEnv *env, jobject obj) {
  freeMatrix();
}

JNIEXPORT void JNICALL
Java_com_xinlan_imageeditlibrary_editimage_fliter_PhotoProcessing_handleSmoothAndWhiteSkin(
    JNIEnv *env, jobject obj, jobject bitmap, jfloat smoothValue,
    jfloat whiteValue) {
...
JNIEXPORT void JNICALL
Java_com_xinlan_imageeditlibrary_editimage_fliter_PhotoProcessing_handleSmooth(
    JNIEnv *env, jobject obj, jobject bitmap, jfloat smoothValue) {
  AndroidBitmapInfo info;
  void *pixels;
  int ret;
...
JNIEXPORT void JNICALL
Java_com_xinlan_imageeditlibrary_editimage_fliter_PhotoProcessing_handleWhiteSkin(
    JNIEnv *env, jobject obj, jobject bitmap, jfloat whiteValue) {
  AndroidBitmapInfo info;
  void *pixels;
  int ret;
...
JNIEXPORT void JNICALL
Java_com_xinlan_imageeditlibrary_editimage_fliter_PhotoProcessing_nativeMosaic(
    JNIEnv *env, jclass type, jobject bitmap, jobject out_bitmap, jint radius) {
  AndroidBitmapInfo info;
  void *pixels;
  int ret;
  ...
  do_mosaic(pixels, out_pixels, info.width, info.height, info.stride,
            out_info.stride, radius);
...
#elif defined(__OHOS__)
void handleSmoothAndWhiteSkin(napi_env env, napi_value bitmap,
                              float smoothValue, float whiteValue) {
  ...
  if ((ret = OHOS::Media::OH_GetImageInfo(env, bitmap, &info)) != 0) {
    LOGI("OH_GetImageInfo() failed ! error=%d", ret);
    return;
  }

  if ((ret = OHOS::Media::OH_AccessPixels(env, bitmap, &pixels)) < 0) {
    LOGI("AndroidBitmap_lockPixels() failed ! error=%d", ret);
    return;
  }
  ...
  OHOS::Media::OH_UnAccessPixels(env, bitmap);
...
void setSmooth(uint32_t *pix, float smoothValue, int width,
               int height) { // 磨皮操作
  if (mIntegralMatrix == NULL || mIntegralMatrixSqr == NULL ||
      mSkinMatrix == NULL) { // 预操作辅助未准备好
    LOGE("not init correctly");
    return;
  }

  LOGE("AndroidBitmap_smooth setSmooth start---- smoothValue = %f",
       smoothValue);
  ...
  for (int i = 1; i < height; i++) {
    for (int j = 1; j < width; j++) {
      ...
      if (mSkinMatrix[offset] == 255) {
        ...
        float m = (mIntegralMatrix[i4] + mIntegralMatrix[i3] -
                   mIntegralMatrix[i2] - mIntegralMatrix[i1]) /
                  squar;

        float v = (mIntegralMatrixSqr[i4] + mIntegralMatrixSqr[i3] -
                   mIntegralMatrixSqr[i2] - mIntegralMatrixSqr[i1]) /
                      squar -
                  m * m;
        float k = v / (v + smoothValue);

        mImageData_yuv[offset * 3] =
            ceil(m - k * m + k * mImageData_yuv[offset * 3]);
      }
    }
  }
  ...
  for (int i = 0; i < h; i++) {
    for (int j = 0; j < w; j++) {
      ...
      if ((RGB.blue > 95 && RGB.green > 40 && RGB.red > 20 &&
           RGB.blue - RGB.red > 15 &&
           RGB.blue - RGB.green > 15) || // uniform illumination
...
void initIntegralMatrix(int width, int height) {
  ...
  for (int i = 1; i < height; i++) {
    ...
    for (int j = 1; j < width; j++) {
      ...
      columnSumSqr[j] +=
          mImageData_yuv[3 * (offset + j)] * mImageData_yuv[3 * (offset + j)];

      mIntegralMatrix[offset + j] =
          mIntegralMatrix[offset + j - 1] + columnSum[j];
      mIntegralMatrixSqr[offset + j] =
          mIntegralMatrixSqr[offset + j - 1] + columnSumSqr[j];
    }
  }
...
void RGBToYCbCr(uint8_t *From, uint8_t *To, int length) {
  ...
  for (i = 0; i < length; i++) {
    ...
    offset = (i << 1) + i;
    To[offset] = (uint8_t)((YCbCrYRI * Red + YCbCrYGI * Green +
                            YCbCrYBI * Blue + HalfShiftValue) >>
                           Shift);
    To[offset + 1] = (uint8_t)(128 + ((YCbCrCbRI * Red + YCbCrCbGI * Green +
                                       YCbCrCbBI * Blue + HalfShiftValue) >>
                                      Shift));
    To[offset + 2] = (uint8_t)(128 + ((YCbCrCrRI * Red + YCbCrCrGI * Green +
                                       YCbCrCrBI * Blue + HalfShiftValue) >>
                                      Shift));
    To[offset + 3] = alpha;
  }
}
